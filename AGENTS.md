...
- A borrow cannot be laundered back into an owner: where a by-value `T` is expected and `T` needs drop, a `ref T` is rejected — in an assignment, a by-value argument, a `return`, a field store, or an enum payload (`isAssignableTo`, and the copy of that rule in `scope_lookup.cpp` used by overload resolution). Reading `T` out of the borrow would give the copy and the borrowed value the same buffer. Types with no drop glue (views, scalars) still copy out normally.
