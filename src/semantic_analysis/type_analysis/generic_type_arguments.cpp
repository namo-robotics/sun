/**
 * Matches read-only parameter shapes to deduce generic type arguments.
 * Keep deduction independent of semantic sessions and declaration lookup.
 * Callers supply resolved facts, request specializations, and report failures.
 */
 ...
 */
InferenceResult<std::vector<TypePtr>> completeTypeArguments(
    const std::vector<std::string>& typeParams,
    std::map<std::string, TypePtr>& bindings,
  ...
  for (const auto& typeParam : typeParams) {
    ...
    if (found == bindings.end() || !found->second) {
      return InferenceFailure{
          InferenceFailure::Kind::UnboundParameter, {}, {}, typeParam};
    }
...
/** Deduce bindings from supplied parameter shapes, keeping the first binding.
 */
template <class Shape>
InferenceResult<std::vector<TypePtr>> deduce(
    const std::vector<std::string>& parameters,
    const std::vector<Shape>& shapes, const std::vector<TypePtr>& arguments,
    const std::vector<TypePtr>& explicitArguments) {
  std::map<std::string, TypePtr> bindings;
  for (size_t i = 0; i < shapes.size() && i < arguments.size(); ++i)
    bindTypeParameters(shapes[i], arguments[i], parameters, bindings);
  return completeTypeArguments(parameters, bindings, explicitArguments);
}
InferenceResult<std::vector<TypePtr>> inferTypeArguments(
    ...
    const std::vector<TypePtr>& explicitArguments) {
  return deduce(parameters, shapes, arguments, explicitArguments);
}
InferenceResult<std::vector<TypePtr>> inferTypeArguments(
    const std::vector<std::string>& parameters,
