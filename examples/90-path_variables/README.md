...
Manifest entries can reference variables with `$NAME`. Here the manifest asks
for `$LIBS/mathlib.moon`, and the build supplies the directory:
...
Variables work in `suns`, `moons` (paths and urls) and `protos` entries
alike, which keeps a manifest portable when libraries live in a different
