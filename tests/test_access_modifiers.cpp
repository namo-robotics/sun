// tests/test_access_modifiers.cpp
...
TEST(AccessModifiers, lexer_public_is_keyword_private_is_identifier) {
...
TEST(AccessModifiers, parser_accepts_public_on_every_item_kind) {
...
TEST(AccessModifiers, parser_rejects_public_inside_function_body) {
  EXPECT_THROW(compileString(R"(
    function main() i32 { public var x: i32 = 1; return x; }
  )"), SunError);
}

TEST(AccessModifiers, parser_rejects_public_on_non_declarations) {
...
TEST(AccessModifiers, formatter_round_trips_public) {
  std::string src =
      "public module m {\n"
      "  public class C {\n"
      "    public var x: i32;\n"
...
TEST(AccessModifiers, serialization_round_trips_visibility) {
  auto parser = Parser::createStringParser(R"(
    public module m {
        public class C { public var x: i32; var y: i32;
                         public function init() {} function h() void {} }
...
TEST(AccessModifiers, public_api_is_reachable_from_outside) {
  auto value = executeString(program(
      "var b = m.make(3); return b.reveal() + b.v + m.pubinner.g() + m.visible;"));
  EXPECT_EQ(value, 106 + 3 + 2 + 8);
}

TEST(AccessModifiers, private_items_reachable_within_module) {
...
TEST(AccessModifiers, root_level_items_are_reachable_everywhere) {
  auto value = executeString(R"(
...
TEST(AccessModifiers, child_module_sees_parent_private_items) {
  auto value = executeString(R"(
    public module a {
...
TEST(AccessModifiers, private_class_denied) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(
...
TEST(AccessModifiers, module_reopened_with_conflicting_visibility_is_error) {
...
TEST(AccessModifiers, private_init_with_public_factory) {
  auto value = executeString(R"(
    public module m {
        public class Token {
            public var id: i32;
...
TEST(AccessModifiers, deinit_is_always_callable) {
  auto value = executeString(R"(
    public module m {
...
TEST(AccessModifiers, private_index_operator_denied) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(compileString(R"(
    public module m {
...
TEST(AccessModifiers, struct_literal_needs_accessible_fields) {
  auto value = executeString(R"(
    public module m {
...
TEST(AccessModifiers, generic_bodies_keep_their_module_context) {
  auto value = executeString(R"(
    public module m {
        function helper() i32 { return 100; }
...
TEST(AccessModifiers, public_interface_member_needs_public_implementation) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(compileString(R"(
    public module m {
...
TEST(AccessModifiers, private_interface_member_only_reachable_in_module) {
  auto value = executeString(R"(
    public module m {
...
TEST(AccessModifiers, inherited_interface_field_keeps_interface_visibility) {
  auto value = executeString(R"(
    public module m {
...
TEST(AccessModifiers, moon_hides_private_items_but_generics_still_work) {
  initTestEnvironment();
...
TEST(AccessModifiers, moon_rejects_private_root_module) {
  initTestEnvironment();
...
TEST(AccessModifiers, stdlib_internals_are_private) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(compileString(R"(
    using sun;
    function main() i32 {
