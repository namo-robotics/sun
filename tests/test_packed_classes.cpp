// tests/test_packed_classes.cpp - Tests for packed_class layout
//
...
TEST(PackedClass, removes_trailing_and_interior_padding) {
...
TEST(PackedClass, unpacked_equivalent_still_pads) {
  auto value = executeString(R"(
    class U {
        var a: u8;
        var b: i32;
...
TEST(PackedClass, all_primitive_widths) {
  auto value = executeString(R"(
    packed_class Wide {
        var a: u8;
        var b: i16;
...
TEST(PackedClass, single_field_matches_unpacked) {
  auto value = executeString(R"(
    packed_class One {
        var a: i32;
        function init() {}
...
TEST(PackedClass, field_write_read_roundtrip) {
  auto value = executeString(R"(
    packed_class P {
        var a: u8;
        var b: i32;
...
TEST(PackedClass, compound_assignment_on_unaligned_field) {
  auto value = executeString(R"(
    packed_class P {
        var a: u8;
        var b: i32;
...
TEST(PackedClass, constructor_initializes_unaligned_fields) {
  auto value = executeString(R"(
    packed_class P {
        var a: u8;
        var b: i32;
...
TEST(PackedClass, methods_read_own_unaligned_fields) {
  auto value = executeString(R"(
    packed_class P {
        var a: u8;
        var b: i32;
...
TEST(PackedClass, passed_by_ref_whole_object) {
...
TEST(PackedClass, nested_packed_class_field) {
  auto value = executeString(R"(
    packed_class Inner {
        var x: u8;
        var y: i32;
...
TEST(PackedClass, nested_packed_field_roundtrip) {
  auto value = executeString(R"(
    packed_class Inner {
        var x: u8;
        var y: i32;
...
TEST(PackedClass, packed_field_inside_unpacked_class_is_allowed) {
  auto value = executeString(R"(
    packed_class Inner {
        var x: u8;
        var y: i32;
...
TEST(PackedClass, generic_specialization_stays_packed) {
  auto value = executeString(R"(
    packed_class Box<T> {
        var tag: u8;
        var value: T;
...
TEST(PackedClass, generic_specializations_differ_by_type_arg) {
  auto value = executeString(R"(
    packed_class Box<T> {
        var tag: u8;
        var value: T;
...
TEST(PackedClass, cannot_create_ref_to_packed_field) {
...
TEST(PackedClass, cannot_pass_packed_field_to_ref_param) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(executeString(R"(
    packed_class P {
        var a: u8;
        var b: i32;
...
TEST(PackedClass, cannot_ref_field_reached_through_packed_owner) {
...
TEST(PackedClass, rejects_array_field) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(executeString(R"(
    packed_class P {
        var a: u8;
        var data: array<i32>;
...
TEST(PackedClass, rejects_non_packed_class_field) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(executeString(R"(
    class Q {
        var x: i32;
        function init() {}
...
TEST(PackedClass, rejects_partial_combined_with_packed) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(executeString(R"(
...
TEST(PackedClass, field_accesses_are_emitted_with_align_1) {
...
TEST(PackedClass, unpacked_field_accesses_keep_natural_alignment) {
  std::string ir = irFor(R"(
    class U {
        var a: u8;
        var b: i32;
...
TEST(PackedClass, packed_remains_a_usable_identifier) {
  auto value = executeString(R"(
    function main() i32 {
        var packed: i32 = 40;
