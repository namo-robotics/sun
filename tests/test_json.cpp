// tests/test_json.cpp - stdlib/json.sun: parsing, access, building, writing
...
TEST(JsonTest, parse_string_escapes) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i32, IError {
        var alloc = make_heap_allocator();
...
TEST(JsonTest, parse_nested_and_access) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i32, IError {
        var alloc = make_heap_allocator();
...
TEST(JsonTest, reader_kind_mismatch_throws) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i32 {
        var alloc = make_heap_allocator();
        var codes: i32 = 0;
...
TEST(JsonTest, parse_errors_report_offsets) {
  auto value = executeStringWithStdlib(withMain(R"(
...
TEST(JsonTest, json_error_carries_offset) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i64 {
...
TEST(JsonTest, deep_nesting_is_rejected) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i32 {
        var alloc = make_heap_allocator();
...
TEST(JsonTest, deep_but_allowed_nesting) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i32, IError {
        var alloc = make_heap_allocator();
        var text = String(alloc, "");
...
TEST(JsonTest, build_and_write_compact) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i32, IError {
        var alloc = make_heap_allocator();
...
TEST(JsonTest, number_formatting) {
  auto value = executeStringWithStdlib(withMain(R"(
...
TEST(JsonTest, control_characters_are_escaped_on_write) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i32, IError {
        var alloc = make_heap_allocator();
...
TEST(JsonTest, iterate_object_members_and_array_items) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i32, IError {
        var alloc = make_heap_allocator();
...
TEST(JsonTest, large_document_builds_and_drops) {
  auto value = executeStringWithStdlib(withMain(R"(
    function main() i32, IError {
        var alloc = make_heap_allocator();
        var total: i64 = 0;
...
TEST(JsonTest, json_value_enum_is_matchable) {
  auto value = executeStringWithStdlib(withMain(R"(
