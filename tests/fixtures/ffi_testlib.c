// tests/fixtures/ffi_testlib.c
//
// Built as a shared library (sun_ffi_testlib) so the FFI tests can link
// against symbols that are NOT already present in the test binary. Testing
// against libc alone would pass even if -l/-L did nothing, because libc is
// already resident in the process.

/** Triples a value through a symbol provided only by the shared test library. */
int sun_ffi_triple(int x) { return x * 3; }

/** Adds two wide integers through a native-library call. */
long long sun_ffi_sum(long long a, long long b) { return a + b; }

/** Counts bytes preceding the terminator of a supplied C string. */
int sun_ffi_count_bytes(const unsigned char* s) {
  int n = 0;
  while (s && s[n]) n++;
  return n;
}

/** A native callback accepting and returning the integer used by FFI tests. */
typedef int (*SunFfiCallback)(int);

static SunFfiCallback sun_ffi_stored_callback;

/** Invokes a supplied callback with the test value. */
int sun_ffi_call_callback(SunFfiCallback callback, int value) {
  return callback(value);
}

/** Retains a callback for a later native-to-Sun call. */
void sun_ffi_store_callback(SunFfiCallback callback) {
  sun_ffi_stored_callback = callback;
}

/** Invokes the retained callback, or returns a failure marker when none is stored. */
int sun_ffi_call_stored_callback(int value) {
  return sun_ffi_stored_callback ? sun_ffi_stored_callback(value) : -1;
}

/** A timestamp-shaped structure filled through an FFI pointer. */
struct SunFfiTS {
  long long sec;
  long long nsec;
};

/** Writes test values into a structure passed by pointer. */
void sun_ffi_fill(struct SunFfiTS* t) {
  t->sec = 7;
  t->nsec = 9;
}

/**
 * By-value struct cases, one per System V classification outcome.
 */
struct SunFfiPair {
  int a, b;
};  // 8B  -> one INTEGER reg
/** An aggregate mixing integer and floating-point fields for ABI tests. */
struct SunFfiMixed {
  int a;
  double b;
};  // 16B -> INTEGER + SSE
/** An aggregate large enough to exercise indirect argument and result passing. */
struct SunFfiBig {
  int a, b, c, d, e;
};  // 20B -> MEMORY (byval/sret)

/* Mutable globals used to test extern variable declarations. */
int sun_ffi_global_value = 17;
int* sun_ffi_global_pointer = &sun_ffi_global_value;
struct SunFfiPair sun_ffi_global_pair = {4, 5};

/** Combines the fields of a small by-value structure for ABI assertions. */
int sun_ffi_take_pair(struct SunFfiPair p) { return p.a * 100 + p.b; }
/** Combines integer and floating-point fields received by value. */
int sun_ffi_take_mixed(struct SunFfiMixed m) { return m.a + (int)m.b; }
/** Sums the fields of an aggregate passed indirectly by the ABI. */
int sun_ffi_take_big(struct SunFfiBig b) { return b.a + b.b + b.c + b.d + b.e; }
/** Returns a small structure populated from the supplied values. */
struct SunFfiPair sun_ffi_make_pair(int a, int b) {
  struct SunFfiPair p = {a, b};
  return p;
}
/** Returns a large structure populated with consecutive values. */
struct SunFfiBig sun_ffi_make_big(int base) {
  struct SunFfiBig b = {base, base + 1, base + 2, base + 3, base + 4};
  return b;
}
/** Combines a by-value structure with the following scalar argument. */
int sun_ffi_pair_then_int(struct SunFfiPair p, int extra) {
  return p.a + p.b + extra;
}
