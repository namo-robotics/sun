// tests/ffi/abi/test_sysv.cpp — System V AMD64 argument classification
//
...
TEST_F(Ffi_Abi_SysV, scalars_are_direct) {
  for (llvm::Type* t : {i8(), i32(), i64(), f32(), f64(), ptr()}) {
    auto lowering = sun::codegen::abi::sysv::lowerArgument(t, dl);
...
TEST_F(Ffi_Abi_SysV, void_return_is_direct) {
  EXPECT_TRUE(
      sun::codegen::abi::sysv::lowerReturn(llvm::Type::getVoidTy(ctx), dl)
          .isDirect());
}

// --- register-class aggregates ----------------------------------------------

TEST_F(Ffi_Abi_SysV, two_ints_coerce_to_one_i64) {
  // struct { int, int } -> void t8(i64)
  auto lowering =
      sun::codegen::abi::sysv::lowerArgument(structOf({i32(), i32()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
  ASSERT_EQ(lowering.pieces.size(), 1u);
  EXPECT_EQ(lowering.pieces[0], i64());
}

TEST_F(Ffi_Abi_SysV, int_then_long_splits_into_i32_and_i64) {
  // struct { int, long } -> void t16(i32, i64)
  // The first eightbyte is half padding, so it narrows to i32.
  auto lowering =
      sun::codegen::abi::sysv::lowerArgument(structOf({i32(), i64()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
...
TEST_F(Ffi_Abi_SysV, two_doubles_stay_in_sse_registers) {
  // struct { double, double } -> void tf16(double, double)
  auto lowering =
      sun::codegen::abi::sysv::lowerArgument(structOf({f64(), f64()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
  ASSERT_EQ(lowering.pieces.size(), 2u);
...
TEST_F(Ffi_Abi_SysV, mixed_int_and_double_uses_one_of_each) {
  // struct { int, double } -> void tmix(i32, double)
  auto lowering =
      sun::codegen::abi::sysv::lowerArgument(structOf({i32(), f64()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
  ASSERT_EQ(lowering.pieces.size(), 2u);
  EXPECT_EQ(lowering.pieces[0], i32());
  EXPECT_EQ(lowering.pieces[1], f64());
...
TEST_F(Ffi_Abi_SysV, two_floats_pack_into_one_sse_eightbyte) {
  // struct { float, float } -> void tff(<2 x float>)
  // Both floats share one SSE eightbyte; clang represents that as a vector,
  // not as a double.
  auto lowering =
      sun::codegen::abi::sysv::lowerArgument(structOf({f32(), f32()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
  ASSERT_EQ(lowering.pieces.size(), 1u);
  EXPECT_EQ(lowering.pieces[0], llvm::FixedVectorType::get(f32(), 2));
}

TEST_F(Ffi_Abi_SysV, single_float_stays_a_float) {
  // struct { float } -> void tf1(float)
  auto lowering = sun::codegen::abi::sysv::lowerArgument(structOf({f32()}), dl);
  ASSERT_EQ(lowering.pieces.size(), 1u);
...
TEST_F(Ffi_Abi_SysV, three_floats_split_into_a_vector_and_a_float) {
  ...
  auto lowering = sun::codegen::abi::sysv::lowerArgument(
...
TEST_F(Ffi_Abi_SysV, float_beside_an_int_is_integer_class) {
  // struct { float, int } -> void tfi(i64): a mixed eightbyte is INTEGER.
  auto lowering =
      sun::codegen::abi::sysv::lowerArgument(structOf({f32(), i32()}), dl);
  ASSERT_EQ(lowering.pieces.size(), 1u);
  EXPECT_EQ(lowering.pieces[0], i64());
}

TEST_F(Ffi_Abi_SysV, float_then_double_uses_separate_eightbytes) {
  // struct { float, double } -> void tfd(float, double)
  auto lowering =
      sun::codegen::abi::sysv::lowerArgument(structOf({f32(), f64()}), dl);
  ASSERT_EQ(lowering.pieces.size(), 2u);
  EXPECT_EQ(lowering.pieces[0], f32());
  EXPECT_EQ(lowering.pieces[1], f64());
}

TEST_F(Ffi_Abi_SysV, single_char_coerces_to_i8) {
  // struct { char } -> void t1(i8)
  auto lowering = sun::codegen::abi::sysv::lowerArgument(structOf({i8()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
  ASSERT_EQ(lowering.pieces.size(), 1u);
  EXPECT_EQ(lowering.pieces[0], i8());
}

TEST_F(Ffi_Abi_SysV, integer_width_is_exact_not_rounded_up) {
  // clang emits the exact occupied width: 3 bytes is i24, not i32.
  auto bytes = [&](unsigned n) {
    return sun::codegen::abi::sysv::lowerArgument(
               structOf({llvm::ArrayType::get(i8(), n)}), dl)
        .pieces[0];
  };
  EXPECT_EQ(
      sun::codegen::abi::sysv::lowerArgument(structOf({i16()}), dl).pieces[0],
      i16());
...
TEST_F(Ffi_Abi_SysV, pointer_field_is_integer_class) {
  auto lowering = sun::codegen::abi::sysv::lowerArgument(structOf({ptr()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
  EXPECT_EQ(lowering.pieces[0], i64());
}

TEST_F(Ffi_Abi_SysV, nested_struct_is_flattened_before_classifying) {
  // struct { struct { int, int } } is classified as if the fields were
  // spelled inline: one INTEGER eightbyte.
  auto inner = structOf({i32(), i32()});
  auto lowering = sun::codegen::abi::sysv::lowerArgument(structOf({inner}), dl);
  ASSERT_TRUE(lowering.isCoerced());
  ASSERT_EQ(lowering.pieces.size(), 1u);
  EXPECT_EQ(lowering.pieces[0], i64());
}
...
TEST_F(Ffi_Abi_SysV, aggregate_over_16_bytes_goes_through_memory) {
  // struct { int a,b,c,d,e; } (20 bytes) -> byval pointer
  auto big = structOf({i32(), i32(), i32(), i32(), i32()});
  auto lowering = sun::codegen::abi::sysv::lowerArgument(big, dl);
  ASSERT_TRUE(lowering.isIndirect());
  EXPECT_EQ(lowering.type, big);
  EXPECT_EQ(lowering.align, 4u);
}

TEST_F(Ffi_Abi_SysV, exactly_16_bytes_still_fits_in_registers) {
  auto lowering =
      sun::codegen::abi::sysv::lowerArgument(structOf({i64(), i64()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
  EXPECT_EQ(lowering.pieces.size(), 2u);
}

TEST_F(Ffi_Abi_SysV, seventeen_bytes_does_not) {
  auto lowering = sun::codegen::abi::sysv::lowerArgument(
      structOf({i64(), i64(), i8()}), dl);
  EXPECT_TRUE(lowering.isIndirect());
}

TEST_F(Ffi_Abi_SysV, field_straddling_an_eightbyte_boundary_goes_to_memory) {
  // A packed { i8, i64 } puts the i64 at offset 1, crossing the boundary.
  auto packed = llvm::StructType::get(ctx, {i8(), i64()}, /*isPacked=*/true);
  EXPECT_TRUE(sun::codegen::abi::sysv::lowerArgument(packed, dl).isIndirect());
}

// --- returns ----------------------------------------------------------------

TEST_F(Ffi_Abi_SysV, small_struct_return_is_coerced_not_indirect) {
  // struct S8 r8(void) -> i64 @r8()
  auto lowering =
      sun::codegen::abi::sysv::lowerReturn(structOf({i32(), i32()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
  ASSERT_EQ(lowering.pieces.size(), 1u);
  EXPECT_EQ(lowering.pieces[0], i64());
}

TEST_F(Ffi_Abi_SysV, large_struct_return_uses_sret) {
  auto big = structOf({i32(), i32(), i32(), i32(), i32()});
  auto lowering = sun::codegen::abi::sysv::lowerReturn(big, dl);
  ASSERT_TRUE(lowering.isIndirect());
  EXPECT_EQ(lowering.type, big);
}
...
TEST_F(Ffi_Abi_SysV, lowered_type_expands_coerced_params_in_place) {
  // void f(int, struct{int,long}, int) -> void f(i32, i32, i64, i32)
  llvm::Type* params[] = {i32(), structOf({i32(), i64()}), i32()};
  auto lowering = sun::codegen::abi::sysv::lowerCSignature(
...
TEST_F(Ffi_Abi_SysV, lowered_type_prepends_the_sret_pointer) {
  // struct S20 f(int) -> void f(ptr sret, i32)
  auto big = structOf({i32(), i32(), i32(), i32(), i32()});
  llvm::Type* params[] = {i32()};
  auto lowering = sun::codegen::abi::sysv::lowerCSignature(big, params, dl);
...
TEST_F(Ffi_Abi_SysV, indirect_param_becomes_a_pointer) {
  auto big = structOf({i32(), i32(), i32(), i32(), i32()});
...
TEST_F(Ffi_Abi_SysV, all_scalar_signature_is_trivial) {
  llvm::Type* params[] = {i32(), f64(), ptr()};
  EXPECT_TRUE(
      sun::codegen::abi::sysv::lowerCSignature(i32(), params, dl).isTrivial());
}

TEST_F(Ffi_Abi_SysV, signature_with_an_aggregate_is_not_trivial) {
  llvm::Type* params[] = {structOf({i32(), i32()})};
  EXPECT_FALSE(sun::codegen::abi::sysv::lowerCSignature(
                   llvm::Type::getVoidTy(ctx), params, dl)
                   .isTrivial());
}
...
TEST_F(Ffi_Abi_SysV, eightbyte_pieces_sit_at_offsets_zero_and_eight) {
  auto lowering =
      sun::codegen::abi::sysv::lowerArgument(structOf({i32(), i64()}), dl);
  ASSERT_TRUE(lowering.isCoerced());
...
TEST_F(Ffi_Abi_SysV, indirect_params_use_byval) {
  auto big = structOf({i32(), i32(), i32(), i32(), i32()});
  auto lowering = sun::codegen::abi::sysv::lowerArgument(big, dl);
  ASSERT_TRUE(lowering.isIndirect());
