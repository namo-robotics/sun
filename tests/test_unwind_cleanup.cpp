...
TEST(UnwindCleanupTest, callee_throw_drops_callers_frame_owner) {
...
TEST(UnwindCleanupTest, unwind_through_two_frames_drops_both) {
  auto value = executeString(withPreamble(R"(
...
TEST(UnwindCleanupTest, local_throw_drops_try_scoped_owner_exactly_once) {
  auto value = executeString(withPreamble(R"(
...
TEST(UnwindCleanupTest, local_throw_from_nested_block_drops_all_left_scopes) {
  auto value = executeString(withPreamble(R"(
    function helper() i32, IError {
      try {
        var a = Owner();
...
TEST(UnwindCleanupTest, moved_owner_not_dropped_twice_on_unwind) {
  auto value = executeString(withPreamble(R"(
    function middle() void, IError {
      var a = Owner();
      var b = a;
...
TEST(UnwindCleanupTest, owner_declared_after_throwing_call_not_dropped) {
  auto value = executeString(withPreamble(R"(
    function middle() void, IError {
      var a = Owner();
      thrower();
...
TEST(UnwindCleanupTest, caught_locally_then_normal_exit_no_extra_drops) {
  auto value = executeString(withPreamble(R"(
    function helper() i32, IError {
      var outside = Owner();
      try {
...
TEST(UnwindCleanupTest, no_owners_unwind_still_works) {
  auto value = executeString(withPreamble(R"(
    function middle() void, IError {
      thrower();
    }
...
TEST(UnwindCleanupTest, thrown_error_object_survives_frame_cleanup) {
