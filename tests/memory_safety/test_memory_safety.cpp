...
TEST(MemorySafety_LifetimeInference, ref_return_from_param_ok) {
  auto value = executeString(R"(
...
TEST(MemorySafety_LifetimeInference, ref_return_from_local_rejected) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(executeString(R"(
...
TEST(MemorySafety_LifetimeInference, ref_return_member_of_param_ok) {
  auto value = executeString(R"(
    class Point {
        var x: i32;
        var y: i32;
...
TEST(MemorySafety_LifetimeInference, ref_return_from_this_ok) {
  auto value = executeString(R"(
    class Container {
        var value: i32;
...
TEST(MemorySafety_LifetimeInference, ref_return_nested_local_rejected) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(executeString(R"(
      function bad() ref i32 {
          if (true) {
              var x: i32 = 10;
...
TEST(MemorySafety_LifetimeInference, ref_return_multiple_params_ok) {
  auto value = executeString(R"(
...
TEST(MemorySafety_LifetimeInference, ref_return_array_element_ok) {
  auto value = executeString(R"(
