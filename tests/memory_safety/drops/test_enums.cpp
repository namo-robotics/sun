...
TEST(MemorySafety_Drops_Enums, owning_payload_dropped_at_scope_exit) {
...
TEST(MemorySafety_Drops_Enums, unit_variant_drops_nothing) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
...
TEST(MemorySafety_Drops_Enums, construction_moves_argument_single_drop) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      var o = Owner(1);
      var h = Holder.Hold(o);
...
TEST(MemorySafety_Drops_Enums, construction_use_after_move_is_error) {
...
TEST(MemorySafety_Drops_Enums, enum_var_creation_moves_source) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
...
TEST(MemorySafety_Drops_Enums, enum_var_creation_use_after_move_is_error) {
  EXPECT_THROW(executeString(withPreamble(R"(
    function main() i32 {
      var a = Holder.Hold(Owner(1));
      var b = a;
...
TEST(MemorySafety_Drops_Enums, assignment_drops_old_value_then_moves) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      var h = Holder.Hold(Owner(1));
...
TEST(MemorySafety_Drops_Enums, assignment_of_unit_variant_drops_old_payload) {
  auto value = executeString(withPreamble(R"(
    function main() i32 {
      var h = Holder.Hold(Owner(1));
      h = Holder.Nothing;
...
TEST(MemorySafety_Drops_Enums, match_binding_borrows_payload_no_extra_drop) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
...
TEST(MemorySafety_Drops_Enums, passing_match_binding_by_ref_is_ok) {
  auto value = executeString(withPreamble(R"(
...
TEST(MemorySafety_Drops_Enums, reassigning_discriminant_inside_arm_is_error) {
  EXPECT_THROW(executeString(withPreamble(R"(
    function main() i32 {
      var h = Holder.Hold(Owner(1));
      match h {
...
TEST(MemorySafety_Drops_Enums, moves_in_one_arm_do_not_poison_other_arms) {
  auto value = executeString(withPreamble(R"(
    function consume(o: Owner) i32 { return o.get_id(); }
    function main() i32 {
      var spare = Owner(9);
...
TEST(MemorySafety_Drops_Enums, enum_field_in_class_dropped_with_class) {
  auto value = executeString(withPreamble(R"(
    class Box {
      var slot: Holder;
      function init() {
...
TEST(MemorySafety_Drops_Enums, enum_field_assignment_drops_old_payload) {
  auto value = executeString(withPreamble(R"(
    class Box {
      var slot: Holder;
      function init() {
...
TEST(MemorySafety_Drops_Enums, nested_owning_enum_drops_recursively) {
  auto value = executeString(withPreamble(R"(
...
TEST(MemorySafety_Drops_Enums, block_scoped_owning_enum_dropped_at_block_exit) {
  auto value = executeString(withPreamble(R"(
    function main() i32 {
      if (true) {
        var h = Holder.Hold(Owner(1));
...
TEST(MemorySafety_Drops_Enums, returned_owning_enum_moves_to_caller) {
  auto value = executeString(withPreamble(R"(
