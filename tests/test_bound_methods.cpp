// tests/test_bound_methods.cpp - Class methods as first-class values
//
...
TEST(BoundMethodTest, pass_method_as_callback) {
...
TEST(BoundMethodTest, receiver_mutated_through_callback) {
  auto value = executeString(R"(
    class Counter {
        var count: i32;
        function init() { this.count = 0; }
...
TEST(BoundMethodTest, stored_in_var_then_called) {
  auto value = executeString(R"(
    class Counter {
        var count: i32;
        function init() { this.count = 0; }
...
TEST(BoundMethodTest, stored_with_type_annotation) {
  auto value = executeString(R"(
    class Counter {
        var count: i32;
        function init() { this.count = 0; }
...
TEST(BoundMethodTest, this_method_as_callback) {
  auto value = executeString(R"(
    function apply(f: (i32) i32, x: i32) i32 {
        return f(x);
    }
...
TEST(BoundMethodTest, ref_receiver) {
  auto value = executeString(R"(
    class Counter {
        var count: i32;
        function init() { this.count = 0; }
...
TEST(BoundMethodTest, overload_picked_by_annotation) {
  auto value = executeString(R"(
    class Calc {
        var last: i32;
...
TEST(BoundMethodTest, overload_picked_by_param_context) {
  auto value = executeString(R"(
    class Calc {
        var last: i32;
        function init() { this.last = 0; }
...
TEST(BoundMethodTest, ambiguous_overload_without_context) {
...
TEST(BoundMethodTest, generic_method_reference_rejected) {
  EXPECT_THROW(executeString(R"(
    class Box {
        function init() {}
...
TEST(BoundMethodTest, throwing_method_into_throwing_param) {
...
TEST(BoundMethodTest, nonthrowing_method_into_throwing_param) {
  auto value = executeStringWithStdlib(R"(
    using sun;
...
TEST(BoundMethodTest, stored_throwing_method_uncaught_rejected) {
...
TEST(BoundMethodTest, bound_method_call_in_loop) {
  auto value = executeString(R"(
    class Counter {
        var count: i32;
        function init() { this.count = 0; }
...
TEST(BoundMethodTest, stdlib_class_method_as_value) {
  auto value = executeStringWithStdlib(R"(
    using sun;
...
TEST(BoundMethodTest, deinit_runs_once_with_bound_method) {
  auto value = executeStringWithStdlib(R"(
    using sun;
