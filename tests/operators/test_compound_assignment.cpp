// tests/operators/test_compound_assignment.cpp - Tests for compound assignment
// operators (+=, -=, *=, /=, %=, &=, |=, ^=, <<=, >>=)
...
TEST(Operators_CompoundAssignment, all_operators_chained) {
...
TEST(Operators_CompoundAssignment, plus_assign_f64) {
  auto value = executeString(R"(
      function main() i32 {
          var x: f64 = 1.0;
          x += 1.5;
...
TEST(Operators_CompoundAssignment, rhs_is_full_expression) {
  auto value = executeString(R"(
      function main() i32 {
          var x: i32 = 10;
          x += 1 + 2;
...
TEST(Operators_CompoundAssignment, unsigned_div_assign) {
  auto value = executeString(R"(
      function main() i32 {
...
TEST(Operators_CompoundAssignment, unsigned_shr_assign) {
  auto value = executeString(R"(
      function main() i32 {
          var x: u32 = 3000000000;
          x >>= 1;
...
TEST(Operators_CompoundAssignment, for_loop_increment) {
  auto value = executeString(R"(
      function main() i32 {
          var total: i32 = 0;
...
TEST(Operators_CompoundAssignment, this_field_in_method) {
  auto value = executeString(R"(
      class Counter {
          var count: i32;
          function init() void {
...
TEST(Operators_CompoundAssignment, member_target) {
  auto value = executeString(R"(
      class Point {
          var x: i32;
          function init() void {
...
TEST(Operators_CompoundAssignment, indexed_target) {
  auto value = executeString(R"(
      function main() i32 {
...
TEST(Operators_CompoundAssignment, indexed_side_effect_index_evaluated_once) {
  auto value = executeString(R"(
...
TEST(Operators_CompoundAssignment, indexed_multidim_side_effect_once) {
  auto value = executeString(R"(
...
TEST(Operators_CompoundAssignment, variable_target_is_compound_node) {
...
TEST(Operators_CompoundAssignment, indexed_target_is_compound_node) {
...
TEST(Operators_CompoundAssignment, side_effect_index_is_single_compound_node) {
...
TEST(Operators_CompoundAssignment, member_target_is_compound_node) {
...
TEST(Operators_CompoundAssignment, class_setindex_target) {
  auto value = executeString(R"(
      class Box {
          var slot: i32;
          var gets: i32;
...
TEST(Operators_CompoundAssignment, inside_generic_function) {
  auto value = executeString(R"(
...
TEST(Operators_CompoundAssignment, byref_captured_variable) {
...
TEST(Operators_CompoundAssignment, greater_equal_split_in_generics) {
  auto value = executeString(R"(
      class Box<T> {
          var v: T;
...
TEST(Operators_CompoundAssignment, not_an_expression_operator) {
