// tests/functions/generic/test_functions.cpp - Tests for generic functions
...
TEST(Functions_Generic, generic_function_with_two_type_params) {
  auto value = executeString(R"(
...
TEST(Functions_Generic, two_specializations) {
  auto value = executeString(R"(
    function identity <T> (x: T) T {
        return x;
    }
...
TEST(Functions_Generic, literal_args_widened_to_i64) {
  auto value = executeString(R"(
...
TEST(Functions_Generic, literal_args_narrowed_to_i8) {
  auto value = executeString(R"(
...
TEST(Functions_Generic, literal_arg_out_of_range_is_error) {
...
TEST(Functions_Generic, body_cannot_see_requesters_locals) {
...
TEST(Functions_Generic, called_from_generic_class_method_defined_above_it) {
  auto value = executeString(R"(
    class Box<T> {
        var v: T;
...
TEST(Functions_Generic, called_from_class_method_defined_above_it) {
  auto value = executeString(R"(
    class Plain {
        var n: i32;
...
TEST(Functions_Generic, called_from_function_defined_above_it) {
  auto value = executeString(R"(
...
TEST(Functions_Generic, generic_calls_generic_defined_below_it) {
  auto value = executeString(R"(
...
TEST(Functions_Generic, mutually_ordered_specializations_share_one_symbol) {
...
TEST(Functions_Generic, ref_type_parameter_receives_an_address) {
  auto value = executeString(R"(
...
TEST(Functions_Generic, ref_type_parameter_from_generic_class_method) {
  auto value = executeString(R"(
    function bump<T>(x: ref T) i32 { return 42; }
    class Box<T> {
        var v: T;
...
TEST(Functions_Generic, parameter_naming_a_generic_class) {
  auto value = executeString(R"(
    class Pair<A> {
        var a: A;
...
TEST(Functions_Generic, module_qualified_call) {
  auto value = executeString(R"(
    public module m {
...
TEST(Functions_Generic, nested_module_qualified_call_from_generic_method) {
  auto value = executeString(R"(
    public module a {
        public module b {
...
TEST(Functions_Generic, type_argument_inferred_from_argument) {
  auto value = executeString(R"(
...
TEST(Functions_Generic, type_arguments_inferred_for_each_parameter) {
  auto value = executeString(R"(
...
TEST(Functions_Generic, type_argument_inferred_through_ref_parameter) {
  auto value = executeString(R"(
    class Point {
        var x: i32;
...
TEST(Functions_Generic, type_argument_inferred_from_generic_class_argument) {
  auto value = executeString(R"(
    class Pair<A> {
        var a: A;
        function init(a: A) { this.a = a; }
...
TEST(Functions_Generic, inference_from_imported_module) {
  auto value = executeString(R"(
    public module m {
        public function twice<T>(x: T) T { return x + x; }
    }
...
TEST(Functions_Generic, uninferable_type_argument_is_error) {
  EXPECT_SUN_ERROR_WITH_MESSAGE(executeString(R"(
