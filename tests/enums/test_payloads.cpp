// tests/enums/test_payloads.cpp - Payload-carrying enums, match destructuring,
...
TEST(Enums_Payloads, ConstructAndMatchSinglePayload) {
...
TEST(Enums_Payloads, UnitVariantOfPayloadEnum) {
  auto value = executeString(R"(
    enum Opt { Some(i32), None }
...
TEST(Enums_Payloads, MultiPayloadAndWildcardBinding) {
  auto value = executeString(R"(
...
TEST(Enums_Payloads, MixedPayloadTypes) {
  auto value = executeString(R"(
...
TEST(Enums_Payloads, RefParamAndReturnByValue) {
  auto value = executeString(R"(
    enum Shape { Circle(f64), Rect(f64, f64), Empty }
...
TEST(Enums_Payloads, MatchCallResultDirectly) {
  auto value = executeString(R"(
    enum Opt { Some(i32), None }
...
TEST(Enums_Payloads, CopyIndependence) {
  auto value = executeString(R"(
    enum Opt { Some(i32), None }
...
TEST(Enums_Payloads, EnumInClassField) {
  auto value = executeString(R"(
...
TEST(Enums_Payloads, ClassPayload) {
  auto value = executeString(R"(
    class Point {
      var x: i32;
      var y: i32;
...
TEST(Enums_Payloads, NestedEnumPayload) {
  auto value = executeString(R"(
    enum Inner { A(i32), B }
...
TEST(Enums_Payloads, EarlyReturnInArm) {
  auto value = executeString(R"(
    enum Opt { Some(i32), None }
...
TEST(Enums_Payloads, BindingShadowsOuter) {
  auto value = executeString(R"(
    enum Opt { Some(i32), None }
...
TEST(Enums_Payloads, CStyleEnumPayloadInVariant) {
  auto value = executeString(R"(
...
TEST(Enums_Payloads, CStyleEnumRegression) {
  auto value = executeString(R"(
    enum Color { Red, Green, Blue }
...
TEST(Enums_Payloads, NonExhaustiveMatchIsError) {
  try {
    executeString(R"(
      enum Shape { Circle(f64), Rect(f64, f64), Empty }
...
TEST(Enums_Payloads, CStyleEnumExhaustivenessEnforced) {
...
TEST(Enums_Payloads, WildcardSatisfiesExhaustiveness) {
  auto value = executeString(R"(
    enum Shape { Circle(f64), Rect(f64, f64), Empty }
...
TEST(Enums_Payloads, PayloadVariantWithoutArgsIsError) {
  EXPECT_THROW(executeString(R"(
    enum Opt { Some(i32), None }
    function main() i32 {
      var x: Opt = Opt.Some;
...
TEST(Enums_Payloads, UnitVariantWithArgsIsError) {
  EXPECT_THROW(executeString(R"(
    enum Opt { Some(i32), None }
    function main() i32 {
      var x = Opt.None(1);
...
TEST(Enums_Payloads, WrongArityConstructionIsError) {
  EXPECT_THROW(executeString(R"(
...
TEST(Enums_Payloads, WrongArityPatternIsError) {
  EXPECT_THROW(executeString(R"(
...
TEST(Enums_Payloads, EqualityOnPayloadEnumIsError) {
  EXPECT_THROW(executeString(R"(
    enum Opt { Some(i32), None }
    function main() i32 {
      var a = Opt.Some(1);
...
TEST(Enums_Payloads, ByValuePayloadEnumParamMoves) {
  auto value = executeString(R"(
    enum Opt { Some(i32), None }
...
TEST(Enums_Payloads, ByValuePayloadEnumParamUseAfterMoveIsError) {
  EXPECT_THROW(executeString(R"(
    enum Opt { Some(i32), None }
...
TEST(Enums_Payloads, DestructuringOnNonEnumIsError) {
  EXPECT_THROW(executeString(R"(
    function main() i32 {
      var x = 5;
      return match x {
...
TEST(Enums_Payloads, NonVariantPatternOnEnumIsError) {
  EXPECT_THROW(executeString(R"(
    enum Opt { Some(i32), None }
    function main() i32 {
      var x = Opt.None;
...
TEST(Enums_Payloads, RecursiveEnumIsError) {
  EXPECT_THROW(executeString(R"(
...
TEST(Enums_Payloads, RefPayloadBorrowsTheReferent) {
  auto value = executeString(R"(
...
// tests/memory_safety/drops/test_enums.cpp for the drop-glue behaviour.
TEST(Enums_Payloads, DeinitClassPayloadCompiles) {
  auto value = executeString(R"(
    class Owner {
      var p: raw_ptr<i8>;
      init() {
...
TEST(Enums_Payloads, EmptyPayloadParensIsError) {
  EXPECT_THROW(executeString(R"(
    enum Bad { Foo(), Bar }
    function main() i32 { return 0; }
  )"),
...
TEST(Enums_Payloads, GlobalPayloadEnumVarIsError) {
  EXPECT_THROW(executeString(R"(
    enum Opt { Some(i32), None }
    var g = Opt.Some(1);
    function main() i32 { return 0; }
