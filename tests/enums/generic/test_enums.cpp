// tests/enums/generic/test_enums.cpp - Generic enums (Stage 2): declaration,
...
TEST(Enums_Generic, OptionEndToEnd) {
...
TEST(Enums_Generic, TwoSpecializationsCoexist) {
  auto value = executeString(R"(
    enum Option<T> { Some(T), None }
...
TEST(Enums_Generic, ResultWithTwoParams) {
  auto value = executeString(R"(
...
TEST(Enums_Generic, ExpectedTypeFillsUnboundParams) {
  auto value = executeString(R"(
    enum Result<T, E> { Ok(T), Err(E) }
...
TEST(Enums_Generic, GenericEnumInClassField) {
  auto value = executeString(R"(
    enum Option<T> { Some(T), None }
...
TEST(Enums_Generic, NestedSpecializationPayload) {
  auto value = executeString(R"(
    enum Option<T> { Some(T), None }
...
TEST(Enums_Generic, PointerPayloadInference) {
  auto value = executeString(R"(
    enum Option<T> { Some(T), None }
...
TEST(Enums_Generic, BareNoneWithoutContextIsError) {
...
TEST(Enums_Generic, WrongArityTypeArgsIsError) {
  EXPECT_THROW(executeString(R"(
    enum Option<T> { Some(T), None }
    function main() i32 {
...
TEST(Enums_Generic, ConflictingInferenceIsError) {
  EXPECT_THROW(executeString(R"(
...
TEST(Enums_Generic, SpecializationExhaustivenessEnforced) {
  EXPECT_THROW(executeString(R"(
    enum Option<T> { Some(T), None }
    function main() i32 {
      var x = Option.Some(1);
