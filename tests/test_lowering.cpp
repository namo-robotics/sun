// tests/test_lowering.cpp — LoweringPass tests
//
...
TEST(LoweringTest, StripsAllParenNodes) {
...
TEST(LoweringTest, IdempotentOnCorpus) {
  auto block = parseProgram(kCorpus);
  ASSERT_NE(block, nullptr);
...
TEST(LoweringTest, NoOpOnParenFreeProgram) {
  const char* src =
      "function f() i32 {\n"
      "    var x: i32 = 1 + 2;\n"
      "    return x;\n"
...
TEST(LoweringTest, ParenStrippingPreservesInner) {
...
TEST(LoweringTest, DesugarsInterpolatedString) {
...
TEST(LoweringTest, DesugaredNodesCarryTemplateLocation) {
...
TEST(LoweringTest, InterpolationSubExpressionPositionsAreAbsolute) {
