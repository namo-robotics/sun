...
TEST(DFATest, AlternationAndRepetition) {
...
TEST(DFATest, WildcardMatchesEveryByte) {
...
TEST(DFATest, NegatedClassesCoverHighBytes) {
...
TEST(DFATest, CaptureOffsetsOnMasterRegex) {
...
TEST(DFATest, LongestMatchWinsOverShorterAlternative) {
  DFA& dfa = Lexer::getTokenDFA();
...
TEST(DFATest, CaptureTracksLongestPerGroup) {
  RegexParser parser;
...
TEST(DFATest, ResetInvalidatesCaptures) {
  RegexParser parser;
  DFA dfa = parser.parse("([0-9]+)");
...
TEST(DFATest, TransitionCacheIsReused) {
  RegexParser parser;
...
TEST(DFATest, StartStateIsNotAccepting) {
...
TEST(DFATest, DeadStateSelfLoops) {
  RegexParser parser;
...
TEST(DFATest, AcceptKindIsLowestTokenKindOnTies) {
