...
TEST(ScopeCleanupTest, if_block_owner_dropped_at_block_exit) {
...
TEST(ScopeCleanupTest, else_block_owner_dropped_at_block_exit) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      if (false) {
        var f = Owner();
...
TEST(ScopeCleanupTest, while_body_owner_dropped_per_iteration) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      var i: i32 = 0;
      while (i < 3) {
...
TEST(ScopeCleanupTest, for_body_owner_dropped_per_iteration) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
...
TEST(ScopeCleanupTest, break_drops_body_owner) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      while (true) {
        var f = Owner();
...
TEST(ScopeCleanupTest, break_from_nested_if_drops_all_jumped_scopes) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      while (true) {
        var b = Owner();
...
TEST(ScopeCleanupTest, continue_drops_body_owner_each_iteration) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
...
TEST(ScopeCleanupTest, return_from_nested_block_drops_outer_scopes) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      var outer = Owner();
      if (true) {
...
TEST(ScopeCleanupTest, return_from_loop_drops_all_live_scopes) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      var a = Owner();
...
TEST(ScopeCleanupTest, explicit_deinit_in_block_no_double_drop) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      if (true) {
        var f = Owner();
...
TEST(ScopeCleanupTest, loop_scoped_init_var_dropped_once_after_loop) {
  auto value = executeString(withPreamble(R"(
    function helper() i32 {
      var before: i32 = 0;
      for (var i: i32 = 0; i < 3; i = i + 1) {
