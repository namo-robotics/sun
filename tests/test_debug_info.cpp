// test_debug_info.cpp — DWARF debug info (-g) tests
//
...
TEST(DebugInfoTest, module_flags_present_with_g) {
...
TEST(DebugInfoTest, subprograms_attached_to_functions) {
  auto driver = compileWithDebug(kSimpleProgram);
  auto& module = driver->getModule();
...
TEST(DebugInfoTest, locals_and_params_have_dbg_declares) {
  auto driver = compileWithDebug(kSimpleProgram);
...
TEST(DebugInfoTest, class_types_described_with_members) {
...
TEST(DebugInfoTest, shadowed_variables_get_lexical_block_scopes) {
...
TEST(DebugInfoTest, moon_bundle_carries_debug_info_into_g_compile) {
  initTestEnvironment();
...
TEST(DebugInfoTest, moon_debug_info_stripped_from_non_g_compile) {
  initTestEnvironment();
  auto imports = getStdlibMoonImports();
  if (imports.empty()) GTEST_SKIP() << "stdlib.moon not built";
...
TEST(DebugInfoTest, no_debug_metadata_without_g) {
  initTestEnvironment();
...
TEST(DebugInfoTest, jit_executes_with_debug_info) {
  initTestEnvironment();
...
TEST(DebugInfoTest, object_file_dwarf_verifies) {
  auto driver = compileWithDebug(kClassProgram);
...
TEST(DebugInfoTest, cross_target_object_dwarf_verifies) {
...
TEST(DebugInfoTest, gdb_breaks_reads_args_and_steps) {
...
TEST(DebugInfoTest, gdb_debugs_jit_executed_code) {
  if (!haveTool("gdb")) GTEST_SKIP() << "gdb not installed";
...
TEST(DebugInfoTest, lldb_breaks_reads_variables_and_steps) {
