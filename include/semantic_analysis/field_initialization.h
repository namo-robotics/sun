 ...
 * assign fields, and it may call the object's own methods to assign them for
 * it — a method called there may only read fields that already have a value,
 * and what it assigns counts towards the constructor's own obligation.
 * Reading an unassigned field, or handing `this` to anything else, is
 * rejected: the object is not a whole value yet. The second phase begins once
 * every field has a value, and behaves like any other method body.
 *
 ...
 * a struct literal has to name every field.
 *
 * Along the way each write is tagged: a write that starts a field's life
 * drops nothing, a write to a field that may or may not hold a value checks
 * the storage at run time, and every other write drops what it replaces.
 */
