...
| `split_nonempty(alloc, sep)` | `(ref HeapAllocator, u8) Vec<String>`, `(ref HeapAllocator, static_ptr<u8>) Vec<String>` | Like `split`, but empty pieces are dropped: adjacent, leading, and trailing separators yield nothing |
| `split_whitespace(alloc)` | `(ref HeapAllocator) Vec<String>` | Split on runs of ASCII whitespace; never yields an empty piece |
