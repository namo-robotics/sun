...
| `get_file_size` | `(path: Path) i64, IError` |
...
| `create_new_session` | `() i32, IError` | `setsid` |
...
|--------|-----------|-------------|
| `parse_json(alloc, text)` | `(ref HeapAllocator, ref String) Json, IError` | Parse a complete document (also accepts a `static_ptr<u8>` literal). Rejects trailing text, leading zeros, control characters in strings, and nesting deeper than 256 |
