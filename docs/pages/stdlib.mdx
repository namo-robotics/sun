...
[Testing](/testing)). They compile only into the test binary, never into
`stdlib.moon`.
...
| Method | Signature | Description |
|--------|-----------|-------------|
| `insert(key, value)` | `(K, V) void throws IError` | Insert or update a key-value pair |
| `get(key)` | `(K) V throws IError` | Get value by key (throws if not found) |
| `find(key)` | `(K) Option<ref V> throws IError` | Get value by key; `None` if not present |
| `get_or_default(key, default)` | `(K, ref V) ref V throws IError` | Get value or return default |
| `remove(key)` | `(K) V throws IError` | Remove and return value (throws if not found) |
| `contains(key)` | `(K) bool throws IError` | Check if key exists |
...
Lookup and insertion methods propagate `ArithmeticError` if bucket arithmetic
fails. Call them inside `try` or a function declared with `throws IError`.
...
| `at(i)` | `(i64) u8 throws IError` | Read a byte; reject indices outside `length()` |
| `set_at(i, val)` | `(i64, u8) void throws IError` | Write a byte; reject indices outside `length()` |
...
| `at(i)` | `(i64) u8 throws IError` | Read a byte; reject indices outside the view |
...
`Poller` wraps portable `poll(2)`. Its integer timeout overload keeps descriptors
...
For a finite wait, pass a `Duration` to receive only the descriptors that
...
var events = poller.wait(create_duration_millis(250));
...
| `wait` | `(timeout: const ref Duration) Vec<Event> throws IError` | Ready descriptors in registration order |
...
`TerminalMode` saves the descriptor's settings when it is constructed and puts
them back when it leaves scope, whether by reaching the end of the block, by
`return`, or by an exception unwinding through it. Call `restore()` to put them
...
Constructing one on a descriptor that is not a terminal throws with `ENOTTY`
and changes nothing.
...
The library never installs a signal handler. Sun cannot make a Sun function a C
signal handler, and a process-wide handler installed by a library would be a
surprise anyway, so what the interrupt keys do is a choice the caller makes
once, when constructing the guard:
...
| `get_size` | `(fd: i32) Size throws IError` | `rows` and `cols`; throws `ENOTTY` on a non-terminal |
| `read_key` | `(fd: i32, timeout: const ref Duration) Option<u8> throws IError` | One byte, `None` on timeout; throws once input has closed |
| `try_read_key` | `(fd: i32) Option<u8> throws IError` | `read_key` with a zero timeout |
| `read_attributes` | `(fd: i32) Termios throws IError` | Snapshot of the line-discipline settings |
...
|----------------|-----------|-------------|
| `init` | `(fd: i32, mode: Mode, signals: SignalPolicy) throws IError` | Save and switch |
| `restore` | `() void throws IError` | Put the saved settings back now; idempotent |
...
try { environment.set("LANG", "C"); } catch (e: ref IError) { }
var here = environment.cwd();
```
...
| `set` | `(name: Name, value: Name) void throws IError` | Mutate libc, then refresh |
| `remove` | `(name: Name) void throws IError` | Mutate libc, then refresh |
...
| `cwd` | `() String throws IError` | Working directory |
| `set_cwd` | `(path: Name) void throws IError` | Change it process-wide |
...
stream.set_nonblocking(true);         // configure the chosen connection
...
listener.set_nonblocking(true);       // configure after binding
listener.listen(128);
var client = listener.accept();
client.set_nonblocking(true);         // accepted streams configure themselves
```
...
It throws when the name does not resolve or has no IPv4 address. Socket
operations also throw the positive libc `errno` value through `IError.code()`;
...
accepting until an operation throws an error for which
...
var addr = parse_ipv4(text);             // from a String; throws if malformed
var any = ipv4_any();                    // 0.0.0.0
var home = ipv4_loopback();              // 127.0.0.1
println(lan.to_string(alloc));           // "192.168.1.10"
stream.connect_addr(lan, 80);            // TCP takes it too
listener.bind_addr(any, 8080);
```
...
occupied address throws an `Error` with the native `EADDRINUSE` code. A
...
var any = ipv4_any();
socket.bind(any, 0);
var port: i32 = socket.local_port();

var dest = Ipv4Addr(192, 168, 1, 20);
socket.send_str_to("ping", dest, 9000);      // a literal
socket.send_to(data, len, dest, 9000);       // raw bytes

var buf = ContiguousBuffer<u8>(alloc, 2048);
var datagram = socket.recv_from(buf);        // one datagram, cut to fit buf
...
A blocking `recv_from` can be given a deadline with
`set_recv_timeout(duration)`; when it expires the call throws an error for
...
```sun
var socket = UdpSocket();
socket.bind(ipv4_any(), 9000, true);
var group = Ipv4Addr(239, 255, 0, 1);
var iface = ipv4_any();
socket.join_multicast(group, iface);
socket.set_multicast_ttl(1);        // how many router hops sends may cross
socket.set_multicast_loop(true);    // hear this host's own sends
socket.leave_multicast(group, iface);
```

IPv4 is the only supported address family for now. TCP sockets have no
...
[Testing](/testing) for the full feature. What user code calls directly:

| Function | Signature | Description |
|----------|-----------|-------------|
| `assert(cond)` / `assert(cond, msg)` | `(bool[, static_ptr<u8>]) void throws IError` | Throw `AssertionError` when false |
| `assert_eq(expected, actual)` | overloaded per primitive, plus `(const ref String, const ref String)` | Throw when the values differ, showing both |
| `assert_ne(a, b)` | overloaded per primitive | Throw when the values are equal |
| `fail(msg)` | `(static_ptr<u8>) void throws IError` | Fail unconditionally |
