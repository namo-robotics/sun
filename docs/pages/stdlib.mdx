...
The value lives in a heap box next to a `sun.thread.Mutex` and an atomic count of the live
...
Free functions in `sun` for moving between `char` and code points.
...
`stdlib/thread.sun` (module `sun.thread`). `spawn` starts an OS thread and
