...
Use `Option` for "might not be there" (lookups, first/last), and `IError`
throwing for actual failures. `Result<T, E>` suits fallible operations whose
error is a value rather than an exception.
