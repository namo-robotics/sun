...
p.get().x;     // Access the underlying value (returns ref T)
...
| Method | Signature | Description |
|--------|-----------|-------------|
| `get()` | `() ref T` | Get a reference to the underlying value |
| `raw()` | `() raw_ptr<T>` | Get the underlying raw pointer (for C interop) |
| `release()` | `() raw_ptr<T>` | Release ownership and return raw pointer |
...
| `get(index)` | `(i64) T, IError` | Get element at index (bounds-checked) |
| `get_unchecked(index)` | `(i64) T` | Get element without bounds checking |
| `set(index, value)` | `(i64, T) void, IError` | Set element at index (bounds-checked) |
| `set_unchecked(index, value)` | `(i64, T) void` | Set element without bounds checking |
...
<Callout type="info">
  Use `get_unchecked` and `set_unchecked` in performance-critical code when you can guarantee valid indices. These skip bounds checking.
</Callout>
...
A generic doubly-linked list backed by heap-allocated nodes. Supports O(1) push/pop at both ends and O(n) indexed access. Implements `IIterable<T, LinkedList<T>>` for use with for-in loops.
...
| Method | Signature | Description |
|--------|-----------|-------------|
| `push_back(value)` | `(T) void` | Append element to the back |
| `push_front(value)` | `(T) void` | Prepend element to the front |
| `pop_back()` | `() T, IError` | Remove and return last element |
| `pop_front()` | `() T, IError` | Remove and return first element |
| `get(index)` | `(i64) T, IError` | Get element at index (O(n) traversal) |
| `set(index, value)` | `(i64, T) void, IError` | Set element at index |
| `remove(index)` | `(i64) T, IError` | Remove and return element at index |
| `first()` | `() T, IError` | Get first element without removing |
| `last()` | `() T, IError` | Get last element without removing |
| `size()` | `() i64` | Current number of elements |
| `isEmpty()` | `() bool` | Check if empty |
| `clear()` | `() void` | Remove all elements and free nodes |
| `iter()` | `() LinkedListIterator<T>` | Get iterator for for-in loops |
...
for (var x: i32 in list) {
    println_i32(x);
}
```

<Callout type="info">
  Use `LinkedList` when you need efficient insertions/removals at both ends. For random access patterns, prefer `Vec<T>` which has O(1) indexing.
</Callout>
