...
- **archives** — Native static libraries (`.a`) to carry inside the bundle
  ...
  meaningful with `--emit-moon`. A bundle built on top of this one carries
  them forward along with the code that calls into them, so a program needs
  only the bundle it imports. See [C FFI](/c-ffi)
