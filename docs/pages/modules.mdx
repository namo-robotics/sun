...
(paths and urls) and `protos` alike:

```sun
manifest {
    suns: ["$SHARED/util.sun"]
    moons: ["$LIBS/stdlib.moon"]
    protos: ["$SCHEMAS/telemetry.proto"]
}
```

Define a variable with `--path-var` or in the environment (the compiler
checks `--path-var` definitions first):

```bash
sun --path-var LIBS=/opt/sunlibs myprogram.sun
LIBS=/opt/sunlibs sun myprogram.sun   # same effect
```

Referencing a variable defined in neither place is a compile error. Variables
are expanded before the path resolution above, so a variable can also hold a
relative directory.
