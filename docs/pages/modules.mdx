...
| `errors.sun` | Standard error types (`EmptyError`, `NotFoundError`, `IndexOutOfBoundsError`, etc.) |
| `linked_list.sun` | Generic doubly-linked list `LinkedList<T>` with O(1) push/pop at both ends |
| `map.sun` | Generic hash map `Map<K, V>` with open addressing |
| `matrix.sun` | `Matrix<T>` and `MatrixView<T>` N-dimensional array classes |
| `mutex.sun` | `Mutex` for thread synchronization (atomic + futex) |
| `print.sun` | Print functions (`println`, `println_i32`, `println_i64`, `println_f64`) |
| `simple_math.sun` | Basic math utilities (`add`, `multiply`) |
| `slice.sun` | `SliceRange` type for slice syntax |
| `string.sun` | Heap-allocated `String` and `StringView` classes |
| `unique.sun` | `Unique<T>` smart pointer with automatic cleanup via `deinit` |
| `vec.sun` | Generic growable array `Vec<T>` with iterator support |
