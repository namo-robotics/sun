...
A bundle's bitcode is compiled for one architecture, and the compiler rejects
one built for another — so each file is named for its target. Take the one
matching yours and install it under the canonical name, since manifests
resolve bundles by exact file name:
