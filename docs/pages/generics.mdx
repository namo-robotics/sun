...
Lambda types instantiate generics like any other type, and the `[ref]`
marker is part of the type: `Box<() -> i32>` and `Box<[ref]() -> i32>` are
two distinct instantiations. One instantiated over a `[ref]` lambda type is
