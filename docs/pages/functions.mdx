...
- A **non-throwing method** can be bound where a throwing lambda type (`(i32) -> i32 throws IError`) is expected; a throwing method requires a throwing lambda type and its calls follow the usual `try`/`throws IError` rules.
