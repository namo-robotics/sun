...
- A lambda with by-ref captures holds pointers into the enclosing frame, so it **cannot be returned** from the function — the captured variables would be dead when used. Passing it to `spawn` is fine: the thread is joined when its handle's scope ends, while those variables are still alive (see [Threads](/threads)).
