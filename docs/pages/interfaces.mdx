...
public module shapes {
    ...
    public class Square implements IShape {
        var side: f64;
        public function init(s: f64) { this.side = s; }
...
Sun provides several builtin interfaces that cannot be redefined. See [Builtin Types](/builtin-types) for details:

- **`IError`** - Error handling interface with `code()` and `message()` methods
- **`IIterator<T, Container>`** - Iterator interface with `hasNext(c: ref Container) bool` and `next(c: ref Container) T`
- **`IIterable<T, Self>`** - Iterable interface with `iter()` method returning an iterator

```sun
// Implementing a builtin interface (self-iterating pattern)
class NumberIterator implements IIterator<i32, NumberIterator> {
    ...
    function hasNext(self: ref NumberIterator) bool {
        return this.current < this.max;
    }

    function next(self: ref NumberIterator) i32 {
