...
The `for-in` loop iterates over objects that implement `IIterator<T, Container>` (or `IIterable<T, Self>`, whose `iter()` produces one). These interfaces come from the standard library (`using sun;`). The iterator's `next` method receives a reference to the container being iterated and returns `Option<T>`; the loop ends at `Option.None`. The loop variable's type must match `T`:

```sun
for (var element: Type in iterable) {
    // body
}
```

**Basic example** - sum using a self-iterating pattern:
...
```sun
function sum_until_five(iter: ref RangeIterator) i32 {
    var sum: i32 = 0;
    for (var x: i32 in iter) {
        if (x == 5) {
            break;  // Exit early
