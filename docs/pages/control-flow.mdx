...
The `for-in` loop iterates over objects that implement `IIterator<T, Container>`. The iterator's `hasNext` and `next` methods receive a reference to the container being iterated:
