...
```sun
var state: i32 = 0;
var old = unsafe { _atomic_cmpxchg_i32(_address_of<i32>(state), 0, 1); };
...
```sun
unsafe { _atomic_store_i32(_address_of<i32>(state), 0); };  // Reset to 0
```
...
```sun
var current = unsafe { _atomic_load_i32(_address_of<i32>(state)); };
```
...
```sun
// One more owner
unsafe { _atomic_fetch_add_i32(_address_of<i32>(count), 1); };

// One fewer. was == 1 means this was the last handle.
var was = unsafe { _atomic_fetch_sub_i32(_address_of<i32>(count), 1); };
```
