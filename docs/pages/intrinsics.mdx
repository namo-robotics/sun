...
function passed as a value; it must not be able to throw (the thread cannot
hand an unwinding error back). The arguments move into the thread.
...
needed. See [Enums](/enums#explicit-values-and-integer-decoding) for an example.
