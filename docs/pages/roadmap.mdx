...
information, go-to-definition and find references today, plus a VS Code
extension in `extensions/vscode-sun`. The remaining navigation features are
the gap.
...
- [x] Find references, across the files of a manifest. Uses inside a `.moon`
      bundle are not listed: bundles carry no function bodies.
