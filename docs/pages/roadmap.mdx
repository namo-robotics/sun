...
- [ ] Peek accessors returning `Option<ref T>` (`Vec.first`/`last`,
      `Map.find`, `LinkedList.first`/`last`) as `const function`: the borrow
      inside the payload would need to become `const ref T` on a constant
      receiver, which means a second `Option` specialization with the same
      layout
