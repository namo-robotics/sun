...
- [ ] Cross-target archives: `tls.moon` carries x86-64 musl archives, so a
      cross build (aarch64 Linux and macOS alike) needs per-target OpenSSL
      archives — the manifest's `target:` block can now select them, what's
      missing is fetching/building the archives per target
