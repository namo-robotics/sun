...
elements inline, and their dimensions are part of the type. Constant
expressions, including calls to pure functions, are evaluated at compile time,
and an array size can be the name of a constant: `array<T, N>`.

- [ ] Generic value parameters: `array<T, const N>`
- [ ] An array size computed by a function call, when the size is used by a
      class field or a function signature, or the function is declared below
      the use
...
- [ ] Generic overloads that differ only in their constraint (one name, same
      parameter shape, `<F: _Lambda>` vs `<F: _Function>`) — duplicate generic
      declarations are rejected until overload resolution supports constraints
      (#194); `spawn` uses the union trait `_Callable` instead
