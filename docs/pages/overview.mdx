...
| Group | Words |
|-------|-------|
| Declarations | `var`, `function`, `lambda`, `class`, `packed_class`, `partial`, `interface`, `enum`, `implements`, `public`, `def`, `declare`, `extern` |
...
`in` (in a `for` loop), `as` (in an `extern` declaration) and `const` (after
`ref`).
