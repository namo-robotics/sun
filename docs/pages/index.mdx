...
On Linux this expects clang, LLVM 20 dev packages, protobuf (`libprotoc-dev`),
Ninja and lld. On a Mac, install the dependencies with Homebrew first —
`brew install llvm@20 protobuf ninja ccache` — and `./build.sh` picks up
Homebrew's LLVM by itself.
