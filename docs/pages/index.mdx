...
Install Homebrew's LLVM (the package's one dependency), then the installer
package from the latest dev build:

```bash
brew install llvm@20
curl -LO https://github.com/namo-robotics/sun/releases/download/dev/sun-0.dev-arm64-apple-darwin.pkg
sudo installer -pkg sun-0.dev-arm64-apple-darwin.pkg -target /
```

This installs `sun`, `sun-lsp` and the standard library under `/usr/local`
(`/usr/local/lib/sun/stdlib.moon` is on the compiler's bundle search path).
To update, install the new package over the old one. The TLS bundle is not
included on macOS yet.
...
On macOS, remove the installed files:

```bash
sudo rm -rf /usr/local/bin/sun /usr/local/bin/sun-lsp /usr/local/lib/sun /usr/local/share/sun
sudo pkgutil --forget com.namo-robotics.sun
```
