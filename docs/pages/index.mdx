...
macOS is supported both as a cross-compilation target from Linux
(`sun --target arm64-apple-darwin --emit-obj`, then link the object on a Mac
with `cc -o prog prog.o -lc++`) and natively by building the compiler from
source as above. Fully static binaries do not exist on macOS, so `-c` links
dynamically there.
