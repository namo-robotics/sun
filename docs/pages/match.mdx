...
See [Ownership and Drops](/enums#ownership-and-drops).

A non-enum match producing an owned value needs a wildcard, or both boolean
patterns, so every input produces a valid result.

Only the first matching arm runs; there is no fallthrough. A wildcard handles
values not covered by preceding arms.
