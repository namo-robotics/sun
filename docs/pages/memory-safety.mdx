...
<Callout type="info">
  Sun is inference-first: most code never writes a lifetime. References are scoped and cannot escape their defining context. Optional Rust-style lifetime names (`'a`, `'this`) exist for one purpose — relating two positions of one signature, so a callee that stores one argument into another can say so (see [Named Lifetimes](#named-lifetimes-relate-two-positions-of-one-signature)).
</Callout>
...
the enclosing scope ends. The value must also never be held by a variable
declared in an outer scope than what it borrows — the holder would keep the
borrowed storage's address past its death:
...
The bound follows the value when it moves: a holder built in an inner scope
may move outward exactly as far as the outermost variable it borrows.
...
### Rule 5b: A `[ref]` Lambda Type Is Bound to a Stack Frame
...
`[ref](…) -> …`, while a plain `(…) -> …` annotation admits only
environment-free lambdas (an environment-free lambda widens into a `[ref]`
...
```sun
function make() [ref]() -> i32 { ... }   // ❌ ERROR: [ref] cannot be a return type
var g: [ref]() -> i32 = ...;             // ❌ ERROR: a global would outlive the frame
```

The property is transitive, exactly like Rule 5: a class with a `[ref]`
field, an enum with a `[ref]` payload, or a container instantiated over a
`[ref]` lambda type (`Vec<[ref]() -> i32>`) is frame-bound as a whole — it
...
The type says the value points into *a* frame, but not *which*, so one more
rule tells the frames apart. A `[ref]` value built in the **current** frame
— a capture-list literal, or a bound method of a local — may only be stored
where the current frame owns the storage: a local object, which then
becomes frame-bound itself. Storing it into a field of `this`, or into an
object reached through a `ref` parameter, would let a callee smuggle its
own frame's environment up into a caller's object, so both are rejected. A
`[ref]` value received **as a parameter** has an environment in an ancestor
frame that outlives this one, which is exactly what lets a method store a
caller-supplied callback in a field:

```sun
class Bus {
    var cb: [ref]() -> i32;
    public method subscribe(cb: [ref]() -> i32) void {
        this.cb = cb;      // ✓ cb comes from an older frame than this call
    }
    public method arm() void {
        var x = 3;
        // ❌ ERROR: the lambda's environment dies with this call
        this.cb = lambda [ref x]() i32 { return x; };
    }
}
```

Within one frame, the destination must also not be declared in an outer
scope than the environment — an inner scope's locals die at its closing
brace, while the destination lives on:
...
Two names declared in the **same** scope are not ordered by this rule: they
drop in reverse declaration order at the scope's end, and the checker does
not chase which of two siblings dies first. Keep a callback and its bus in
the order that lets the bus drop first, or drop the bus explicitly.
...
Call sites never write lifetime arguments — the compiler binds each name to
...
interface's names exactly. Everything elided keeps its pre-lifetime
meaning; a bare `[ref]` return stays banned, since it names no frame.
