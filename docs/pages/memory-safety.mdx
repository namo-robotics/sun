...
A block may hold several statements; the last one is its value.

The compiler rejects these outside a block: calling an `extern "C"` function,
reading a field through a `raw_ptr<T>` where `T` is a class, `_malloc`, `_free`,
`_load_i64`, `_store_i64`, the `_atomic_*` and `_futex_*` intrinsics, and the
`__`-prefixed syscall intrinsics. The other raw-memory intrinsics — `_load<T>`,
`_store<T>`, `_to_ref<T>`, `_memcpy`, `_ptr_offset` — are just as unchecked, and
the standard library writes every one of them inside `unsafe { }` too. See
[Compiler Intrinsics](/intrinsics) and [C FFI](/c-ffi).
