...
class TS {
    var sec: i64;
    var nsec: i64;
    function init(sec: i64, nsec: i64) { this.sec = sec; this.nsec = nsec; }
}
...
Structs by value work too, in both directions. The compiler applies System V
argument classification, so a small struct travels in registers and a large one
through memory, matching what a C compiler emits:
...
- Argument classification implements x86-64 System V only. Other targets need
  their own rules.
