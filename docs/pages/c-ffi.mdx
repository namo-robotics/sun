...
- Argument classification implements x86-64 System V and AArch64 AAPCS64
  ...
  `sun --target aarch64-linux-gnu -c -o prog program.sun` when a cross
  toolchain (`aarch64-linux-gnu-gcc`, or clang with a `--sysroot`) is
  installed, or stop at `--emit-obj` and link on the target machine.
