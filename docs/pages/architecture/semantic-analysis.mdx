...
The role of semantic analysis is essentially to assign types to every symbol and expression,
and to validate semantic consistency. For example if we have a statement such as

```sun
var x = foo(bar());
```

analysis checks that the return type of `bar` is compatible with the 
...
[`ImportLibraryDeclarationsPass`](/compiler-api/classsun_1_1semantic__analysis_1_1passes_1_1ImportLibraryDeclarationsPass)
