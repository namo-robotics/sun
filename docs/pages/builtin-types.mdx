...
A handle to a spawned OS thread that will return a value of type `T`. Created via the `spawn` keyword; the type is inferred and cannot be written as an annotation.
...
The handle owns the running thread: if it goes out of scope without `join()`, the thread is joined there.
