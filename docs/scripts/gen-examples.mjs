...
// compiles and runs. Three outputs:
//
...
//  - docs/generated/<name>.source.mdx: just the program, for an example whose
//    README carries `docs-source: <page>`. The named page writes its own prose
//    around the program and splices this in, so a walkthrough can be built on
//    a program CI actually compiles and runs (index.mdx imports
//    generated/00-hello.source.mdx). Unlike `docs-page`, this does not take
//    the example off the Examples page — the two keys can be used together or
//    apart.
//
...
function checkHostImports(name, key, page, partialFile) {
  ...
  if (!existsSync(host)) {
    throw new Error(`examples/${name}/README.md names ${key} "${page}", but pages/${page}.mdx does not exist.`)
  }
  if (!readFileSync(host, 'utf8').includes(`generated/${partialFile}`)) {
    throw new Error(
      `examples/${name}/README.md names ${key} "${page}", but pages/${page}.mdx never imports generated/${partialFile}, ` +
        `so the example would appear nowhere. Add the import, or drop the frontmatter.`,
    )
  }
}
...
const sourcePartials = examples.filter((e) => e.meta['docs-source'])
...
for (const example of partials) {
  ...
  checkHostImports(name, 'docs-page', page, `${name}.mdx`)
  const out = join(GENERATED_DIR, name + '.mdx')
  writeFileSync(out, renderPartial(example))
  console.log(`Generated ${out} for the ${page} page.`)
}

for (const example of sourcePartials) {
  const name = basename(example.dir)
  const page = example.meta['docs-source']
  checkHostImports(name, 'docs-source', page, `${name}.source.mdx`)
  const out = join(GENERATED_DIR, name + '.source.mdx')
  writeFileSync(out, renderSourcePartial(example))
  console.log(`Generated ${out} for the ${page} page.`)
}
